...
class JutPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('jut.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `jut`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM jut WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO jut (date, title, link, content)"
